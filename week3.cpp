...
#define pitch_derivative_gain 1.0
#define pitch_integral_gain 0.1
...
int main (int argc, char *argv[]) {
	...
	while(run_program == 1)
	{
		...
		if (run_program == 0){
			...
			set_motors(joystick_data.thrust);
		}
	}
...
void set_motors(int thrust) {
	// thrust ranges from 0-255, where 0 is all the way up and 255 is all the way down
	...
	// printf("%d; %d; %10.5f; %10.5f; %10.5f\n", motor_commands[0], motor_commands[1], thrust_desired, pitch_t * 10, pitch_desired * 10);
