...
int main (int argc, char *argv[]) {
	...
	while(run_program == 1)
	{
		...
		if (run_program == 0){
			...
			pid_control(joystick_data.thrust);
...
int setup_imu() {
	...
	else
	{
		...
		// wiringPiI2CWriteReg8(accel_address, 0x40, 0x08); //high speed filtered accel
...
void pid_control(int thrust) {
	...
	printf("%d; %d; %10.5f; %10.5f\n", motor_commands[0], motor_commands[1], pitch_t*10.0, pitch_angle_calibrated*10.0);
